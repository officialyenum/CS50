...
function Player:init(map)
    ...
    self.message = love.graphics.print("Congrats!!!", VIRTUAL_WIDTH / 2, VIRTUAL_HEIGHT / 2);
...
function Player:checkPoleFlag()
    if self.map:tileAt(self.x, self.y).id == POLE_FLAG or 
        self.map:tileAt(self.x, self.y).id == POLE_MIDDLE or 
            self.map:tileAt(self.x, self.y).id == POLE_BOTTOM then
        -- TRIGGER MESSAGE
        -- Change the displayed text
        self.map = self.map:init();
    end

    if self.map:tileAt(self.x + self.width - 1, self.y + self.height - 1).id == POLE_FLAG or 
        self.map:tileAt(self.x + self.width, self.y + self.height).id == POLE_MIDDLE or 
            self.map:tileAt(self.x + self.width - 1, self.y + self.height - 1).id == POLE_BOTTOM then
        -- TRIGGER MESSAGE
        -- Change the displayed text
        self.map = self.map:init();
    end
